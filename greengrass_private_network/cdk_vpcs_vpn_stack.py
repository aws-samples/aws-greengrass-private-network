...
class CdkVpcsVpnStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        remote_vpc = ec2.Vpc(
            ...
            subnet_configuration=[
                ec2.SubnetConfiguration(
                    subnet_type=ec2.SubnetType.PUBLIC, name="Public"
                ),
                ec2.SubnetConfiguration(
        ...
        remote_security_group = ec2.SecurityGroup(
            self,
            "PingRemoteBastion",
            vpc=remote_vpc,
        ...
        remote_security_group.add_ingress_rule(
            ec2.Peer.ipv4("172.16.0.0/16"), ec2.Port.all_icmp(), "pingable"
        )

        remote_host = ec2.BastionHostLinux(
            self,
            "RemoteBastionHost",
            vpc=remote_vpc,
        ...
        cloud_vpc = ec2.Vpc(
            self,
            "PrivateCloudVPC",  # This is the greengrass VPC
            cidr="172.16.1.0/24",
            subnet_configuration=[
                ec2.SubnetConfiguration(
                    subnet_type=ec2.SubnetType.PRIVATE_ISOLATED, name="Isolated"
                )
            ],
        )
        ...
        cloud_security_group = ec2.SecurityGroup(
            self,
            "PingCloudBastion",
            vpc=cloud_vpc,
            description="Allow pings",
        ...
        # ipv4("172.16.0.0/16")
        cloud_security_group.add_ingress_rule(
            ec2.Peer.any_ipv4(), ec2.Port.all_icmp(), "pingable"
        )
        cloud_security_group.add_ingress_rule(
            ec2.Peer.any_ipv4(), ec2.Port.tcp(22), "allow ssh"
        )

        cloud_host = ec2.BastionHostLinux(
            self,
            "CloudBastionHost",
            vpc=cloud_vpc,
            subnet_selection=ec2.SubnetSelection(
